...
# Load the CSV of non-zero lead-lag results
results_df = pd.read_csv("clean_lead_lag_results_cross_country.csv")

# Parse start and end timestamps from the Period column
results_df["start"] = pd.to_datetime(results_df["Period"].apply(lambda x: x.split(" → ")[0]))
results_df["end"] = pd.to_datetime(results_df["Period"].apply(lambda x: x.split(" → ")[1]))
results_df = results_df.sort_values("start").reset_index(drop=True)

# Update hypotheses to match your ETFs
hypothesis_pairs = {
...
# Download price data in 7-day chunks (Yahoo limit for 1-minute)
data_chunks = []
...
# Group consecutive periods within 1 hour for plotting
groups = []
current_group = [0]
...
folder_name = f"lead_lag_plots_cross_country_{timestamp}"
...
for group in groups:
    ...
    hypotheses_in_group = results_df.loc[group, ["Hypothesis", "Lag (min)"]].drop_duplicates()
    
    for _, row in hypotheses_in_group.iterrows():
        hypothesis = row["Hypothesis"]
        lag_minutes = int(row["Lag (min)"])
...
print(f"\nPlots saved in folder: {folder_name}")
